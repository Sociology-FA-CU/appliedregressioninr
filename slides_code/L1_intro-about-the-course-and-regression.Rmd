...
-   Careful, more terms does not necessarily mean more predictors!
...
-   Detect relationships that can be expressed as linear combination of our predictors
-   But it can only catch relationships you tell it to look for...
...
-   Example: Did the voter turnout in the last elections differ across administrative regions with the same level of unemployment?
...
-   Example: Does exposure to political advertisements increases citizens willingness to vote?
...
-   $\small Sum\:of\:squares=-2^2+1.1^2+\\\small2.8^2+(-4)^2+\\\small1.6^2+1.8^2+\\\small(-0.3^2)+(-0.2^2)+\\\small(-0.1^2)+(-0.7^2)=35.3$
...
-   For details on how OLS is calculated, see @fox2015[pp. 83]
