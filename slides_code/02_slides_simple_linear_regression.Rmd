...
- Understanding basic concepts and building block for regression analysis
- Building and interpreting simple linear regression in R
...
collection process had been re-done, ..." [@gelman2020, pp. 50]
...
- BUT we only have one sample from the sampling distribution! I.e. one value of sample mean, one value of given regression coefficient... we observe no variation. We need to estimate (or conceptualize) it from the observed data. (For regression coefficients, this is where assumptions kick in, we will talk about it later.)
...
- t-distribution: similar to normal (symmetric), but heavier tails, three parameters: center, scale, degrees of freedom 


## Statistical significance and hypothesis testing

- conventional wisdom: statistical significance is p-value less than 0.05, relative to some null hypothesis (hypothesis of no effect)
...
- p-value: "the probability under a specified statistical model that a statistical summary of the data (e.g., the sample mean difference between two compared groups) would be equal to or more extreme than its observed value" CITE ASA
...
CITE: ASA statement which links to other relevant literature as well https://doi.org/10.1080/00031305.2016.1154108
...
- Usually: standardize all variables (z-scores).
- Maybe better: standardizing by subtracting the mean and dividing by 2 standard deviations (not 1) - direct comparability with untransformed binary predictors
