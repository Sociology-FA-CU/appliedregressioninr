...
-   Standard deviation of the distribution of a given estimate is called standard error = $ \frac{\sigma}{n} $. Confidence intervals are extension of standard errors, see picture below (beta-hat is the estimate of the regression coefficient beta from the data).
...
$$
var(x) = E[(x_i - \mu_x)^2] = E[(x_i - \mu_x)*(x_i - \mu_x)] = \frac{(x_i - \mu_x)*(x_z-\mu_z)}{n}
$$
<br>

$$
cov(x) =E[(x_i - \mu_x)*(y_i - \mu_y)] = \frac{(x_i - \mu_x)*(y_i-\mu_y)}{n}
$$
...
-   The formula $y = \alpha + \beta_1*x_1$  represents the line
