...
ggplot(aes(x = agea, y = vote), data = vote) +
  ...
  geom_vline(xintercept = 25, linetype = "dashed") +
  geom_vline(xintercept = 55, linetype = "dashed") +
  geom_smooth(method = "lm", se = F, formula = y ~ lspline(x, c(25,55))) +
  theme_ggeffects()
```
...
ggplot(un, mapping = aes(x = GDPperCapita, y = infantMortality)) +
  ...
  geom_smooth(method = "lm", se = F, formula = y ~ ns(x, df = 6),
              mapping = aes(color = "Cubic splines")) +
  geom_smooth(method = "lm", se = F, formula = y ~ poly(x, 5),
              mapping = aes(color = "Simple polynimial")) +
  theme_ggeffects() +
...
```{r simple-polynomials-equivalence, fig.width=7}
ggplot(data = vote, aes(x = agea, y = vote)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE,
              formula = y ~ poly(x, 2),
              mapping = aes(color = "Simple cubic polynomial")) +
  geom_smooth(method = "lm", se = FALSE,
              formula = y ~ ns(x, 2),
              mapping = aes(color = "Cubic spline with only outer knots")) +
  labs(color = element_blank()) +
  theme_ggeffects() +
