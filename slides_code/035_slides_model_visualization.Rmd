...
- The countries whose squared democratic index is higher by 1, while their base democratic index is 0 and if they are post soviet, have on average 5.1 years higher life expectancy (???).

## Marginal effects

- Once variables are transformed, interpreting relationships between them becomes difficult
- A solution is to interpret models visually, instead of relying on regression coefficients tables
- This can be done by utilizing marginal effects

## Marginal effects

- Marginal effect = expected value of the dependent variable for given values of an independent variable, while other independent variables are held constant
- In R, can be computed using `ggeffects` package (which in turn partialy depends on `effects`, and `emeans`)
...
- Best used for large number of predictor, especially for dummy variables
