...
-   *R^2^* represents the proportion of variance of the depended variable, which can be predicted using the the independent variables
...
-   Some say *R^2^* is the proportion of variance "explained" by the model, but this has unfortunate implications about causality
...
-   Intercept only model is one with zero predictors, predictions are made solely based on the grand average
...
-   *R^2^* basically tells us how much we reduced the prediction error by adding our predictors

-   if *R^2^* = 0, then our model is as "good" as if we had no predictor at all

-   if *R^2^* = 1, then we predict our data perfectly
...
-   This is like a classic ANOVA, but instead of comparing inter- and intra-group variance, we are comparing residual variances of two models
...
-   All the classic limitations of null hypothesis testing
    ...
    -   Differences does not matter, if they are practically unimportant -\> rejecting null hypothesis is by itself not particularly interesting
...
-   This makes it behave unintuitively when used for other than predictive models (and sometimes even for them)
-   There are 4 gotchas you need to be careful about

# Gotcha 1 - *R^2^* depends on the number of predictors
...
-   Notice that the more the predictors the higher *R^2^*, even if the dependent variable $y$ is not related to any of the independent variables $x_i$

## *R^2^* depends on the number of predictors

-   *R^2^* will raise everytime we add a new predictor
-   This happens because even if there is no correlation between two variables in the population, sample correlation will never be exactly 0 (due to sampling error)
-   Consequently, to some extent we are predicting random noise

## *R^2^* depends on the number of predictors

-   Solution is to adjust *R^2^* when comparing models with different number of predictors
-   *Adjusted R^2^* [@henri1961] controls was the number of predictors in the model (represented by the degrees of freedom)

$$
R^{2}_{adj} = 1 - (1 - R^{2}) * \frac{no. \: of \: observations - 1}{no. \: of \: observations - no. \: of \: parameters - 1}
$$

-   *Adjusted R^2^* only raises when the contribution of a new predictor is bigger than what we would expect from pure chance

-   Conculsion: Use *Adjusted R^2^* when you are comparing models with different number of predictors
...
-   The relationsip between $x$ and all $y_i$ is the same:
...
-   Notice that *R^2^* varies widely, despite all models being perfectly specified and describing the relationship correctly
...
-   Low *R^2^* does not neccesarily mean the estimates are incorrect (biased)

-   Low *R^2^* can simply mean that we cannot explain a social phenomenum in its entirity, but that is almost never our goal.
...
# Gotcha 3 - *R^2^* depends on the range of independent variables

## *R^2^* depends on the range of independent variables

-   Consider variables $y$ and $x$
-   $x$ is normaly distributed wiht mean of 50 and standard deviation of 15
-   The relationship between them is $y = 3*x$ with residual standard deviation of 50
-   What would happen, if limited the range of $x$ to \<35;65\> ?
...
-   *R^2^* will naturally gets lower as we restrict the range of independent variables

-   However this does not mean the model is any less valid, just that predictive power is lower
...
# Gotcha 4 - Model with higher *R^2^* do not neccesarily provide a better estimate of regression coefficients

## *R^2^* and coefficient bias

-   Imagine we have representative sample of adults and want to learn what is the relationship between intelligence (`IQ`) and work diligence (`diligence`) like. We also know if our respondents have a university degree (`degree`)
...
-   `degree` is related to both `IQ` and `diligence` - only the top 20% most intelligent or the top 20% most diligent people will obtain a university degreee

-   Should we control for `degree` or not?
...
-   This also happens with mediators and other confounders
    -   See the first few chapters of @pearl2016 for details
    

- Conclusion: If the goal of an analysis is the interpretation of regression coefficients (either for causal inference or testing a hypothesis generated by theory), do not select variables based on *R^2^*. Doing so will sooner or later lead to bias and an incorrect conclusion. Use theory and common sense instead

## Limitations of *R^2^*

- To summarize:
  - *R^2^* has to be adjusted when comparing models with different number of predictors
