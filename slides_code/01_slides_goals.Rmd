...
```{r anonymous-regression, message=FALSE, warning=FALSE, fig.width=4.5, fig.pos="center"}
...
binary <- countries %>%
  mutate(postsoviet_n = if_else(postsoviet == "yes",
                                1, 0)) %>% 
  ggplot(aes(x= postsoviet_n, y = life_exp)) +
  geom_jitter(height = 0, width = 0.1)+
  geom_smooth(method = "lm", se = FALSE) +
  scale_x_continuous(breaks = c(0,1),
                     labels = c("No", "Yes")) +
  labs(x = "predictor variabel on x axis\nhere: Is country postsoviet?",
       y = element_blank()) 
...
```{r, out.width="650px"}
knitr::include_graphics("../images/regression-plane-example.png")
```
...
```{r dag-packages, echo=TRUE, eval=FALSE}
...
```{r dag-simple-example, echo=TRUE, fig.height=3}
dagify(y ~ x + z + q ,
       x ~ z,
       q ~ x,
       w ~ y + x) %>% 
...
```{r dag-ggplo2-example, echo=TRUE, eval=FALSE}
dagify(y ~ x + z + q ,
       x ~ z,
       q ~ x,
       w ~ y + x) %>% 
...
```{r dag-custom-name-position, echo=TRUE, eval=FALSE}
dagify(y ~ x + z + q ,
       x ~ z,
       q ~ x,
       w ~ y + x,
...
```{r colliders-graphically, echo=TRUE, eval=FALSE}
our_dag <- dagify(y ~ x + z + q ,
                  x ~ z,
                  q ~ x,
                  w ~ y + x,
